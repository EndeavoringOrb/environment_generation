...
sf::Vector2f calculate_screen_coordinate(float h_fov_deg, float v_fov_deg, const sf::Vector3f& camera_pos, const sf::Vector3f& camera_dir, const sf::Vector3f& point_pos, const sf::Vector2f& screen_size) {
    ...
    sf::Vector2f vec = sf::Vector2f((x + 1) * screen_size.x / 2, (1 - y) * screen_size.y / 2);
    cout << vec.x << " " << vec.y << endl;
    return sf::Vector2f((x + 1) * screen_size.x / 2, (1 - y) * screen_size.y / 2);
}

void addPoint(const int& i, sf::VertexArray& vertices, const vector<sf::Vector3f>& points, const sf::Vector3f& point, const sf::Vector3f& camera_position, const sf::Vector3f& camera_direction, const float& view_radius, const float& scale, const int& offset_x, const int& offset_y, const int& HFOV, const int& VFOV, const int& screen_x, const int& screen_y) {
    if (isinCircle(point, camera_position, view_radius))
    {
        sf::Vector2f screen_point;
        screen_point = calculate_screen_coordinate(HFOV,VFOV,camera_position,camera_direction,point,sf::Vector2f(screen_x,screen_y));
        vertices.append(sf::Vertex(sf::Vector2f(screen_point.x, screen_point.y),getColor(points[i].y)));
...
int main()
{
    ...
    sf::Vector3f camera_direction = sf::Vector3f(0.0f,0.0f,0.0f);
    const int HFOV = 90;
    const int VFOV = 90;
            ...
            else if (event.type == sf::Event::KeyPressed)
            {
                if (event.key.code == sf::Keyboard::Num1)
                {
                    ...
                    for (std::size_t i = 0; i < points.size(); ++i)
                    {
                        ...
                        addPoint(i, vertices, points, point, camera_position, camera_direction, view_radius, scale, offset_x, offset_y, HFOV, VFOV, window_x, window_y);
                    }
                }
